...
const AdminDashboard = () => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        const newResource = {
          id: data.file._id, // Use the ID from the backend
          branch: data.file.branch,
          semester: data.file.semester,
          subject: data.file.subject,
          fileName: data.file.fileName,
          fileSize: `${(file.size / 1024 / 1024).toFixed(1)} MB`,
          uploadDate: new Date(data.file.uploadedAt).toISOString().split("T")[0],
          uploadTime: new Date(data.file.uploadedAt).toLocaleTimeString(),
        };
  ...
  const fetchResources = async () => {
    try {
      // Remove subject from URL if not needed, or provide default
      const url = subject 
        ? `http://localhost:5000/api/files/${encodeURIComponent(branch)}/${semester}/${encodeURIComponent(subject)}`
        : `http://localhost:5000/api/files/${encodeURIComponent(branch)}/${semester}`;
      ...
      const data = await response.json();
      const formattedResources = data.map((file) => ({
        id: file._id,
        branch: file.branch,
  ...
  // Fetch resources when branch, semester, or subject changes
  useEffect(() => {
    fetchResources();
